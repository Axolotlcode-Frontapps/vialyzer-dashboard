...
import { DRAWING_COLORS } from "./drawing-utils";
...
export class DrawingCore {
	...
	// Update sizes when media or display changes
	updateSizes(
		mediaSize: { width: number; height: number },
		displaySize: { width: number; height: number }
	): void {
	...
	displayToMediaCoords(point: Point): Point {
		...
		const scaleX = this.#mediaSize.width / this.#displaySize.width;
		const scaleY = this.#mediaSize.height / this.#displaySize.height;
	...
	mediaToDisplayCoords(point: Point): Point {
		if (this.#mediaSize.width === 0 || this.#mediaSize.height === 0)
			return point;

		const scaleX = this.#displaySize.width / this.#mediaSize.width;
		const scaleY = this.#displaySize.height / this.#mediaSize.height;
	...
	createDrawingElement(
		...
		return {
			id: elementId,
			type: drawingMode as "line" | "area" | "curve" | "rectangle" | "circle",
			points: [mediaPoint],
			color:
				DRAWING_COLORS[drawingMode as keyof typeof DRAWING_COLORS] || "#000000",
			completed: false,
			layerId,
		};
	}
			...
			if (element.id === dragState.elementId) {
				...
				if (dragState.pointIndex !== null) {
					if (element.type === "rectangle" && newPoints.length >= 2) {
						...
						newPoints[dragState.pointIndex] = mediaPoint;
					}
				}
	...
	drawElement(
		...
		if (
			...
			if (direction && displayPoints.length >= 2) {
				...
				if (element.type === "line") {
					...
					arrowPoint = {
						x:
							displayPoints[lastIdx - 1].x +
							(displayPoints[lastIdx].x - displayPoints[lastIdx - 1].x) * t,
						y:
							displayPoints[lastIdx - 1].y +
							(displayPoints[lastIdx].y - displayPoints[lastIdx - 1].y) * t,
					};
				}

				this.drawArrow(ctx, arrowPoint, displayEnd, element.color, 15);
			}
		}
		...
		if (element.text && element.completed) {
	...
	drawElementText(
		...
		if (!element.text || displayPoints.length === 0) return;

		const textData = element.text;
		let textPoint: Point;
		...
		if (textData.backgroundColor) {
			...
			ctx.globalAlpha = textData.backgroundOpacity;
			ctx.fillRect(
		...
		ctx.fillText(textData.content, textPoint.x, textPoint.y);
	}
	...
	generateMatrix(elements: DrawingElement[]): MediaMatrix {
		const matrix: number[][] = new Array(this.#mediaSize.height)
			.fill(null)
			.map(() => new Array(this.#mediaSize.width).fill(0));
		...
		elements.forEach((element) => {
			...
			if (element.type === "line" && element.points.length >= 2) {
				...
				for (
					...
					y <= Math.min(this.#mediaSize.height - 1, Math.floor(y2));
					y++
				) {
					for (
						let x = Math.max(0, Math.floor(x1));
						x <= Math.min(this.#mediaSize.width - 1, Math.floor(x2));
						x++
					) {
						matrix[y][x] = 4;
					}
	...
	#drawLineInMatrix(
		...
		while (true) {
			if (
				x >= 0 &&
				x < this.#mediaSize.width &&
				y >= 0 &&
				y < this.#mediaSize.height
			) {
				matrix[y][x] = value;
			}
